...
export class OperationService {
  ...
  */
  transfer(from: string, transactions: any, fee: number, keys: KeyPair, tokenContract: string = ''): Observable<any> {
    return this.getHeader()
      .pipe(flatMap((header: any) => {
        return this.http.get(this.nodeURL + '/chains/main/blocks/head/context/contracts/' + keys.pkh + '/counter', {})
          .pipe(flatMap((actions: any) => {
            return this.http.get(this.nodeURL + '/chains/main/blocks/head/context/contracts/' + keys.pkh + '/manager_key', {})
              .pipe(flatMap((manager: any) => {
                ...
                const fop = this.createTransactionObject(header.hash, counter, manager, transactions, keys.pkh, keys.pk, from, fee, tokenContract);
  ...
  createTransactionObject(hash: string, counter: number, manager: string, transactions: any,
    pkh: string, pk: string, from: string, fee: number, tokenContract: string = ''): any {
    const fop: any = {
      branch: hash,
      contents: []
    };
    ...
    for (let i = 0; i < transactions.length; i++) {
      ...
      if (tokenContract) {
        console.log('Invoke contract: ' + tokenContract);
